function restore(){
  ...
  $("#pause").replaceWith('<a class="btn btn-primary one" id="pause">정지</a>');
...
$(document).ready(function(){
  ...
  $(document).on("click", "#pause:not(.disabled)", function(){
    if($(this).hasClass("resume")){
      Fr.voice.resume();
      $(this).replaceWith('<a class="btn btn-primary one" id="pause">정지</a>');
    }else{
      Fr.voice.pause();
      $(this).replaceWith('<a class="btn btn-primary one resume" id="pause">다시시작</a>');
    }
  });
  ...
  $(document).on("click", "#play:not(.disabled)", function(){
    ...
    p1.then(
        function() {
          $("#record, #stop, #save").removeClass("disabled");
